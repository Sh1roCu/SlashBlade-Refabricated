...
public class BladeItemEntityRenderer extends ItemEntityRenderer implements ItemEntityRendererExtension {
    ...
    private void renderBlade(ItemEntity itemIn, float entityYaw, float partialTicks, PoseStack matrixStackIn,
        ...
        if (itemIn instanceof BladeItemEntity bladeItem) {
            try (MSAutoCloser msac = MSAutoCloser.pushMatrix(matrixStackIn)) {
                ...
                ResourceLocation modelLocation = CapabilitySlashBlade.BLADESTATE.maybeGet(current)
                        .map((state) -> state.getModel().orElseGet(bladeItem::getModel)).orElseGet(bladeItem::getModel);
                ResourceLocation textureLocation = CapabilitySlashBlade.BLADESTATE.maybeGet(current)

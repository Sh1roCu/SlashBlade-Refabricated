...
public class SummonedSwordArts {
    ...
    public void onInputChange(InputCommandEvent event) {
        ...
        var bladeState = CapabilitySlashBlade.getBladeState(blade).orElse(new SlashBladeState(blade));

        if (bladeState.isBroken() || bladeState.isSealed()
                || !SwordType.from(blade).contains(SwordType.BEWITCHED))
            return;

        var holder = sender.registryAccess().lookupOrThrow(Registries.ENCHANTMENT).getOrThrow(Enchantments.POWER);
        int powerLevel = EnchantmentHelper.getItemEnchantmentLevel(holder, blade);
        if (powerLevel <= 0)
            return;
    ...
    private void performSpiralSwords(int powerLevel, final Long pressTime, LivingEntity rawEntity) {
        ...
        if (alreadySummoned) {
            ...
            // summon
            CapabilitySlashBlade.getBladeState(entity.getMainHandItem()).ifPresent((state) -> {

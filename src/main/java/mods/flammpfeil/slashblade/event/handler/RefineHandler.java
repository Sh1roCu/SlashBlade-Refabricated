...
public class RefineHandler {
    ...
    public void onAnvilUpdateEvent(AnvilUpdateEvent event) {
        ...
        if (CapabilitySlashBlade.getBladeState(base).isEmpty()) {
            return;
        }
        ...
        CapabilitySlashBlade.getBladeState(result).ifPresent(s -> refineResult.set(s.getRefine()));
        ...
        if (CapabilitySlashBlade.getBladeState(result).isPresent()) {
            ISlashBladeState state = CapabilitySlashBlade.getBladeState(result).orElse(new SlashBladeState(result));
    ...
    public void onAnvilRepairEvent(AnvilRepairEvent event) {
        ...
        int before = CapabilitySlashBlade.getBladeState(base).map(ISlashBladeState::getRefine).orElse(0);
        int after = CapabilitySlashBlade.getBladeState(output).map(ISlashBladeState::getRefine).orElse(0);

...
//
//public class AdvancementsRecipeRenderer implements PlaceRecipeHelper<Ingredient> {
//
//    private static final Minecraft MCINSTANCE = Minecraft.getInstance();
//    private static final Identifier GUI_TEXTURE_CRAFTING_TABLE = Identifier.withDefaultNamespace(
//            "textures/gui/container/crafting_table.png");
//    private static final Identifier GUI_TEXTURE_FURNACE = Identifier.withDefaultNamespace(
//            "textures/gui/container/furnace.png");
//    private static final Identifier GUI_TEXTURE_BLAST_FURNACE = Identifier.withDefaultNamespace(
//            "textures/gui/container/blast_furnace.png");
//    private static final Identifier GUI_TEXTURE_SMOKER = Identifier.withDefaultNamespace(
//            "textures/gui/container/smoker.png");
//    private static final Identifier GUI_TEXTURE_SMITHING = Identifier.withDefaultNamespace(
//            "textures/gui/container/smithing.png");
//    private static final Identifier GUI_TEXTURE_ANVIL = Identifier.withDefaultNamespace("textures/gui/container/anvil.png");
//
//    @Override
//    public void addItemToSlot(Ingredient ingredient, int slotIn, int maxAmount, int y, int x) {
...
//
//    public static Identifier currentRecipe = null;
//
...
//
//    static class DummyAnvilRecipe implements Recipe<RecipeInput> {
...
//            original = recipe;
//
//            RegistryFriendlyByteBuf pb = new RegistryFriendlyByteBuf(PacketByteBufs.create(), MCINSTANCE.level.registryAccess());
//            SmithingTransformRecipe.Serializer ss = new SmithingTransformRecipe.Serializer();
//            ss.streamCodec().encode(pb, original);
//
//            Ingredient.CONTENTS_STREAM_CODEC.decode(pb);
//            nonnulllist.set(0, Ingredient.CONTENTS_STREAM_CODEC.decode(pb));
...
//    }
//
//    static RecipeHolder<?> overrideDummyRecipe(RecipeHolder<?> original) {
//
//        if (!(original.value() instanceof SmithingTransformRecipe))
//            return original;
//
//        if (original.id().getPath().startsWith("anvilcrafting")) {
//            return new RecipeHolder<>(original.id(), new DummyAnvilRecipe((SmithingTransformRecipe) original.value()));
//        } else {
//            return new RecipeHolder<>(original.id(), new DummySmithingRecipe((SmithingTransformRecipe) original.value()));
//        }
//    }
//
...
//
//        public RecipeView(RecipeType<?> recipeType, Identifier background, List<Vec3i> slots) {
...
//        if (icon == null || !(icon.has(DataComponents.CUSTOM_DATA) && icon.get(DataComponents.CUSTOM_DATA).copyTag().contains("Crafting"))) {
//            clearGhostRecipe();
//            return;
//        }
//        getInstance().setGhostRecipe(Identifier.parse(icon.get(DataComponents.CUSTOM_DATA).copyTag().getString("Crafting")));
//    }
//
//    void setGhostRecipe(Identifier loc) {
//        if (Objects.equals(loc, currentRecipe)) return;
//        currentRecipe = loc;//减少性能消耗
//        Optional<RecipeHolder<?>> recipe = MCINSTANCE.level.getRecipeManager().byKey(loc);
//        if (recipe.isEmpty()) {
...
//
//        gr.clear();
//        RecipeHolder<?> iRecipe = recipe.get();
//        iRecipe = overrideDummyRecipe(iRecipe);
//        gr.setRecipe(iRecipe);
//        currentView = typeRecipeViewMap.get(iRecipe.value().getType());
//        if (currentView == null || currentView.slots.size() <= 0) {
//            SlashBlade.LOGGER.warn("[Achievement Recipe Render] The GUI display of the current recipe type is not supported: {}", iRecipe.value().getType());
//            clearGhostRecipe();
//            return;
//        }
//
...
//        gr.addIngredient(Ingredient.of(iRecipe.value().getResultItem(null)), outputSlot.getX(), outputSlot.getY());
//        this.placeRecipe(3, 3, outputslotIndex, iRecipe, iRecipe.value().getIngredients().iterator(), 1);
//    }
//
...
//             */
//            int padding = 5;
//            gg.renderFakeItem(gr.getRecipe().value().getToastSymbol(), xCorner + padding, yCorner + padding);
//
...
//    public void onDrawScreenPost(Screen screen, GuiGraphicsExtractor drawContext, int mouseX, int mouseY, float tickDelta) {
